/* This program is an extension of my level 41 implementation of Indexing Operand City
 * Here I add a custom conversion to BlockOffset that converts from Direction to BlockOffset
 * 
 * 
 * Objectives:
 ...
 * 
 * Add a custom converstion to BlockOffset that converts from Direction to BlockOffset
 * 
 */
...
BlockOffset convertedDirection = (BlockOffset)direction;
...
Console.WriteLine($"Converted direction offset: ({convertedDirection.RowOffset}, {convertedDirection.ColumnOffset})");
...
public record BlockOffset(int RowOffset, int ColumnOffset)
{
    ...
    {
        if (direction == Direction.North) return new BlockOffset(-1, 0);
        if (direction == Direction.South) return new BlockOffset(+1, 0);
        if (direction == Direction.East) return new BlockOffset(0, +1);
        else return new BlockOffset(0, -1);
    }
}
